...
export class TrophyScene extends Phaser.Scene {
  ...
  private buildRoom(): void {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      this.add
        .image(x * DISPLAY_TILE + DISPLAY_TILE / 2, DISPLAY_TILE / 2, PATTERNS_KEY, frame)
        .setScale(TILE_SCALE)
        .setDepth(1);
      ...
      this.add
        .image(
          x * DISPLAY_TILE + DISPLAY_TILE / 2,
          (rows - 1) * DISPLAY_TILE + DISPLAY_TILE / 2,
          PATTERNS_KEY,
          frame,
        )
        .setScale(TILE_SCALE)
    ...
    for (let y = 1; y < rows - 1; y++) {
      ...
      this.add
        .image(DISPLAY_TILE / 2, y * DISPLAY_TILE + DISPLAY_TILE / 2, PATTERNS_KEY, frame)
        .setScale(TILE_SCALE)
        .setDepth(1);

      this.add
        .image(
          (cols - 1) * DISPLAY_TILE + DISPLAY_TILE / 2,
          y * DISPLAY_TILE + DISPLAY_TILE / 2,
          PATTERNS_KEY,
          frame,
        )
    ...
    for (let i = 3; i < cols - 3; i += 4) {
      this.add
        .image(i * DISPLAY_TILE, DISPLAY_TILE / 2, PATTERNS_KEY, PATTERNS.DOOR_ALT)
        .setScale(TILE_SCALE)
        .setDepth(3);
    }
  }
